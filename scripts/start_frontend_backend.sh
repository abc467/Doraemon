...
FRONTEND_BACKEND_MANUAL_DRIVE_REQUIRE_COMBINED_STATUS="${FRONTEND_BACKEND_MANUAL_DRIVE_REQUIRE_COMBINED_STATUS:-${MANUAL_DRIVE_REQUIRE_COMBINED_STATUS:-false}}"
...
validate_manual_drive_mode() {
  ...
  for variable_name in \
    ...
    if [[ "${!variable_name}" != "true" ]]; then
      echo "[ERROR] enabled manual drive requires ${variable_name}=true" >&2
      return 1
    fi
  done
}
