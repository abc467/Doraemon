...
validate_commercial_vehicle_identity() {
  ...
  normalize_boolean_variable RUNTIME_DEPTH_CAMERA_PAUSE_AND_RECOVER_ON_STALE
...
log_effective_runtime_parameters() {
  ...
  runtime_log_status "Orbbec: start=${RUNTIME_START_DEPTH_CAMERAS} require_topics=${RUNTIME_REQUIRE_DEPTH_CAMERA_TOPICS} require_identities=${DORAEMON_REQUIRE_DEPTH_CAMERA_IDENTITIES} watchdog=${RUNTIME_DEPTH_CAMERA_WATCHDOG_ENABLE}(pause_and_recover=${RUNTIME_DEPTH_CAMERA_PAUSE_AND_RECOVER_ON_STALE} stale=${RUNTIME_DEPTH_CAMERA_STALE_TIMEOUT}s cooldown=${RUNTIME_DEPTH_CAMERA_RECOVERY_COOLDOWN}s) obstacle_tracking=${RUNTIME_ENABLE_DEPTH_OBSTACLE_TRACKING} obstacle_sources(left=${RUNTIME_ENABLE_DEPTH_LEFT_CAM} right=${RUNTIME_ENABLE_DEPTH_RIGHT_CAM} front=${RUNTIME_ENABLE_DEPTH_UP_CAM}) left=${RUNTIME_ORBBEC_CAMERA1_SERIAL_NUMBER}@${RUNTIME_ORBBEC_CAMERA1_USB_PORT} right=${RUNTIME_ORBBEC_CAMERA2_SERIAL_NUMBER}@${RUNTIME_ORBBEC_CAMERA2_USB_PORT} front=${RUNTIME_ORBBEC_CAMERA3_SERIAL_NUMBER}@${RUNTIME_ORBBEC_CAMERA3_USB_PORT}"
