  ...
  while (ros::ok())
  {
    ...
    if (head_type[0] == TYPE_IMU)
    {
      ...
      else if (device_type_ == 1)    //imu单品ROS标准下的坐标变换
      {
        ...
        imu_data.angular_velocity.y = -imu_frame_.frame.data.data_pack.gyroscope_y;
        imu_data.angular_velocity.z = -imu_frame_.frame.data.data_pack.gyroscope_z;
        imu_data.linear_acceleration.x = imu_frame_.frame.data.data_pack.accelerometer_x; //减去偏移误差
        imu_data.linear_acceleration.y = -imu_frame_.frame.data.data_pack.accelerometer_y - 0.05;
        imu_data.linear_acceleration.z = -imu_frame_.frame.data.data_pack.accelerometer_z;
        // 增加协方差矩阵
        boost::array<double, 9> orientationArray = {1e6, 0, 0, 
        					    0, 1e6, 0, 
        					    0, 0, 1e-6};
        boost::array<double, 9> angular_velocityArray = {01e6, 0, 0, 
        						  0,  1e6,  0, 
        						  0,  0,  1e-6};
        boost::array<double, 9> linear_accelerationArray = {1e3,  0, 0,
        						 	0,  1e3,  0, 
        						 	0, 0,  1e3};
        imu_data.orientation_covariance = orientationArray;
        imu_data.angular_velocity_covariance = angular_velocityArray;
        imu_data.linear_acceleration_covariance = linear_accelerationArray;        
      }
