...
class CartographerRuntimeManagerNode:
    """Legacy runtime bridge for manual activation/relocalize helpers.

    Formal product flow relies on operators using the external Cartographer UI
    to load maps and relocalize manually. This node is kept only for optional
    legacy/manual tooling and should not be treated as a required v1 main path.
    """
    ...
    def __init__(self):
        ...
        self.startup_activate_map = bool(rospy.get_param("~startup_activate_map", False))
        ...
        rospy.loginfo(
            "[cartographer_runtime_manager] ready legacy_activate=%s legacy_relocalize=%s reload=%s",
    ...
    def _update_runtime_state(
        ...
        *,
        ...
        resolved_map_name = str(map_name if map_name is not None else current.map_name or "").strip()
    ...
    def _resolve_asset(self, *, map_name: str = "") -> Optional[dict]:
        if map_name:
            return self._store.resolve_map_asset(
    ...
    def _handle_activate(self, req):
        ...
        asset = self._resolve_asset(map_name=map_name)
        if not asset:
    ...
    def _handle_relocalize(self, req):
        rospy.logwarn_throttle(
            30.0,
            "[cartographer_runtime_manager] legacy relocalize helper invoked; formal flow should relocalize from external Cartographer UI",
        )
    ...
    def _on_startup_timer(self, _evt):
        if not self.startup_activate_map:
            rospy.loginfo("[cartographer_runtime_manager] startup auto activation disabled")
            return
        asset = None
        try:
            if self.initial_map_name:
                asset = self._resolve_asset(map_name=self.initial_map_name)
            if asset is None:
            ...
            if not ok:
                ...
                rospy.loginfo(
                    "[cartographer_runtime_manager] startup map loaded %s",
                    asset.get("map_name"),
                )
        except Exception as e:
