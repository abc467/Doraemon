...
class CoveragePlannerActionServer:
    ...
    def execute_cb(self, goal):
        ...
        try:
            ...
            active_asset = None
            runtime_map_name, runtime_map_version = get_runtime_map_scope()
            try:
                if runtime_map_name:
                    active_asset = self.store.resolve_map_asset_version(
                        map_name=str(runtime_map_name or ""),
                        map_version=str(runtime_map_version or ""),
            ...
            if (active_asset is None) and (not self.allow_legacy_unscoped_plan):
                return abort("MAP_SCOPE_MISSING", "planner requires an active static map asset before saving plans")
            ...
            plan_id = self.store.save_plan(
                ...
                map_version=str((active_asset or {}).get("map_version") or ""),
