...
class StaticMapManagerNode:
    """Optional static-map publisher for legacy/local workflows.

    The formal v1 product path uses the external Cartographer UI for runtime map
    loading. This node is only a helper when a local static-map service is still
    needed outside the main production flow.
    """
    ...
    def __init__(self):
        ...
        rospy.loginfo(
            "[static_map_manager] ready legacy_activate=%s db=%s",
    ...
    def _load_from_active(self):
        ...
        if self.initial_map_name:
            asset = self._store.resolve_map_asset(
        ...
        try:
            ...
            rospy.loginfo(
                "[static_map_manager] loaded active map=%s id=%s",
                asset.get("map_name"),
                asset.get("map_id"),
            )
