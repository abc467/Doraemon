...
void ObstaclesCritic::initialize()
{
  ...
  param_exists &= nh_.param(param_prefix + "consider_footprint", consider_footprint_, false);
  param_exists &= nh_.param(param_prefix + "cost_power", power_, 1);
  param_exists &= nh_.param(param_prefix + "repulsion_weight", repulsion_weight_, 0.5f);
  param_exists &= nh_.param(param_prefix + "critical_weight", critical_weight_, 20.0f);
