...
// cartographer会将传入进来的landmark id 给reindex
// ,现将landmark的id放在visualization_msgs::Marker中mark.text一起发布由landmark程序接受。
visualization_msgs::Marker CreateLandmarkMarker(int landmark_index,
  ...
  marker.text = label;  // 配合landmark发布的topic中消息类型的id存放在此处
...
void DeleteTrajectoryMarkers(
    const int trajectory_id, const size_t highest_marker_id,
    std::vector<visualization_msgs::Marker>* markers) {
  visualization_msgs::Marker marker;
  marker.ns = absl::StrCat("Trajectory ", trajectory_id);
  marker.action = visualization_msgs::Marker::DELETE;
  for (size_t marker_id = 0; marker_id <= highest_marker_id; ++marker_id) {
    marker.id = marker_id;
    markers->push_back(marker);
  }
}
...
void MapBuilderBridge::DelTrajectory(uint32_t trajectory_id) {
...
void MapBuilderBridge::RunOptimization() {
  LOG(INFO) << "Running optimization...";
  map_builder_->pose_graph()->RunOptimizationOnce();
}
...
void MapBuilderBridge::SubmapToProto(
    int32_t trajectory_id, int32_t submap_index,
...
visualization_msgs::MarkerArray BuildTrajectoryNodeList(
    cartographer::mapping::MapBuilderInterface* const map_builder,
    const NodeOptions& node_options,
    std::unordered_map<int, size_t>* const trajectory_to_highest_marker_id,
    const bool include_frozen_trajectories) {
  visualization_msgs::MarkerArray trajectory_node_list;
  const auto node_poses = map_builder->pose_graph()->GetTrajectoryNodePoses();
  std::set<int> included_trajectory_ids;
  ...
  for (const int trajectory_id : node_poses.trajectory_ids()) {
    if (!include_frozen_trajectories &&
        map_builder->pose_graph()->IsTrajectoryFrozen(trajectory_id)) {
      continue;
    }
    included_trajectory_ids.insert(trajectory_id);
  ...
  for (const auto& constraint : constraints) {
    if (included_trajectory_ids.count(constraint.node_id.trajectory_id) == 0) {
      continue;
    }
    if (constraint.tag ==
  ...
  for (const int trajectory_id : node_poses.trajectory_ids()) {
    if (included_trajectory_ids.count(trajectory_id) == 0) {
      continue;
    }
    visualization_msgs::Marker marker =
        CreateTrajectoryMarker(trajectory_id, node_options.map_frame);
    ...
    if (trajectory_to_highest_marker_id->count(trajectory_id) == 0) {
      (*trajectory_to_highest_marker_id)[trajectory_id] = current_last_marker_id;
    } else {
      marker.action = visualization_msgs::Marker::DELETE;
      while (static_cast<size_t>(marker.id) <=
             (*trajectory_to_highest_marker_id)[trajectory_id]) {
  ...
  std::vector<int> trajectory_ids_to_delete;
  for (const auto& entry : *trajectory_to_highest_marker_id) {
    if (included_trajectory_ids.count(entry.first) == 0) {
      DeleteTrajectoryMarkers(entry.first, entry.second,
                              &trajectory_node_list.markers);
      trajectory_ids_to_delete.push_back(entry.first);
    }
  }
  for (const int trajectory_id : trajectory_ids_to_delete) {
    trajectory_to_highest_marker_id->erase(trajectory_id);
  }
...
visualization_msgs::MarkerArray MapBuilderBridge::GetTrajectoryNodeList() {
  return BuildTrajectoryNodeList(map_builder_.get(), node_options_,
                                 &trajectory_to_highest_marker_id_, true);
}

visualization_msgs::MarkerArray MapBuilderBridge::GetActiveTrajectoryNodeList() {
  return BuildTrajectoryNodeList(map_builder_.get(), node_options_,
                                 &active_trajectory_to_highest_marker_id_,
                                 false);
}
...
visualization_msgs::MarkerArray MapBuilderBridge::GetLandmarkPosesList() {
  ...
  for (const auto& id_to_pose : landmark_poses) {
    landmark_poses_list.markers.push_back(CreateLandmarkMarker(
        ...
        id_to_pose.second, node_options_.map_frame, id_to_pose.first));
  }
...
cartographer::mapping::MapBuilderInterface* MapBuilderBridge::buider() {
  return this->map_builder_.get();
}
