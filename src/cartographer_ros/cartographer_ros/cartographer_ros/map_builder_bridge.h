...
class MapBuilderBridge {
 public:
   ...
   * 不会被回环检测与位姿图优化所更改, 其每一帧位姿间的坐标变换不会改变
   *
   * global frame
   * 是表达被回环检测与位姿图优化所更改后的坐标系, 当有新的优化结果可用时,
   * 此坐标系与任何其他坐标系之间的转换都会跳变. 它的z轴指向上方,
   * 即重力加速度矢量指向-z方向, 即由加速度计测得的重力分量沿+z方向.
   */
  ...
  void DelTrajectory(uint32_t trajectory_id);
  ...
  visualization_msgs::MarkerArray GetActiveTrajectoryNodeList();
  visualization_msgs::MarkerArray GetLandmarkPosesList();
  visualization_msgs::MarkerArray GetConstraintList();

  SensorBridge* sensor_bridge(int trajectory_id);
  cartographer::mapping::MapBuilderInterface* buider();
 ...
 private:
  ...
  std::unordered_map<int, size_t> active_trajectory_to_highest_marker_id_;
};
