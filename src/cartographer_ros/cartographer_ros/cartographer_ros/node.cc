...
Node::Node(
 ...
 // Step: 1 声明需要发布的topic
  ...
  active_trajectory_node_list_publisher_ =
      node_handle_.advertise<::visualization_msgs::MarkerArray>(
          kActiveTrajectoryNodeListTopic, kLatestOnlyPublisherQueueSize);
...
void Node::PublishTrajectoryNodeList(
  ...
  if (trajectory_node_list_publisher_.getNumSubscribers() == 0 &&
      active_trajectory_node_list_publisher_.getNumSubscribers() == 0) {
    return;
  }
  ...
  if (active_trajectory_node_list_publisher_.getNumSubscribers() > 0) {
    active_trajectory_node_list_publisher_.publish(
        map_builder_bridge_.GetActiveTrajectoryNodeList());
  }
}
...
void Node::FinishAllTrajectoriesInROS() {
  for (const auto& entry : map_builder_bridge_.GetTrajectoryStates()) {
    if (entry.second == TrajectoryState::ACTIVE) {
      const int trajectory_id = entry.first;
      CHECK_EQ(this->FinishTrajectoryInROS(trajectory_id), true);
    }
  }
}
...
bool Node::FinishTrajectoryInROS(const int trajectory_id) {
  auto client = this->node_handle_
                    .serviceClient<::cartographer_ros_msgs::FinishTrajectory>(
                        "/finish_trajectory");
  ::cartographer_ros_msgs::FinishTrajectory srv;
  srv.request.trajectory_id = trajectory_id;
  client.call(srv);
  return srv.response.status.code == cartographer_ros_msgs::StatusCode::OK;
}

void Node::RunOptimization() { map_builder_bridge_.RunOptimization(); }
...
bool Node::SerializeState(const std::string& filename,
                          const bool include_unfinished_submaps) {
  absl::MutexLock lock(&mutex_);
  return map_builder_bridge_.SerializeState(filename,
