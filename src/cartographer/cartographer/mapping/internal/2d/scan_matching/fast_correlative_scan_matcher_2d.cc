...
PrecomputationGrid2D::PrecomputationGrid2D(
  ...
  for (int y = 0; y != limits.num_y_cells; ++y) {
    ...
    // y * stride grid的索引 x + width 的坐标范围是 [1, width-1]
    ...
    // 再加上 y * stride grid的索引 x + width 的坐标范围是 [width,
    // limits.num_x_cells-width-1]
    ...
    // Step: 3 滑动窗口正在划出, 一次减少一个值, 所以intermediate的宽度比grid多width-1
    // x + width - 1 + y * stride 的范围是 [limits.num_x_cells-1,
    // limits.num_x_cells+width-1] 再加上 y * stride grid 的索引 x的范围是
    // [limits.num_x_cells-width, limits.num_x_cells-1]
...
PrecomputationGridStack2D::PrecomputationGridStack2D(
  ...
  std::vector<float> reusable_intermediate_grid;
  auto limits = grid.limits().cell_limits();
...
bool FastCorrelativeScanMatcher2D::MatchWithSearchParameters(
  ...
  // LOG(WARNING) << "RotatedCount=" << rotated_scans.size();
  ...
  // LOG(WARNING) << "Candidate Score=" << best_candidate.score;
...
Candidate2D FastCorrelativeScanMatcher2D::BranchAndBound(
  ...
  for (const Candidate2D& candidate : candidates) {
    // LOG(WARNING) << "CandidateScore="<<candidate.score;
