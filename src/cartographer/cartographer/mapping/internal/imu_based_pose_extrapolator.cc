...
void ImuBasedPoseExtrapolator::AddOdometryData(
  ...
  if (!odometry_data_.empty() && odometry_data.time <= odometry_data_.back().time) {
    LOG_EVERY_N(WARNING, 100)
        << "Dropping odometry data with non-increasing time. Current time: "
        << odometry_data.time << ", latest accepted time: "
