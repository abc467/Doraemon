...
void OptimizationProblem2D::AddOdometryData(
  ...
  if (odometry_data_.HasTrajectory(trajectory_id)) {
    const auto latest_odometry =
        std::prev(odometry_data_.EndOfTrajectory(trajectory_id));
    if (odometry_data.time <= latest_odometry->time) {
      LOG_EVERY_N(WARNING, 100)
          << "Dropping odometry data for trajectory " << trajectory_id
          << " with non-increasing time. Current time: " << odometry_data.time
          << ", latest accepted time: " << latest_odometry->time;
      return;
    }
  }
