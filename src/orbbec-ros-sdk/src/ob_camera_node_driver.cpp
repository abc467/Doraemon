...
void OBCameraNodeDriver::startDevice(const std::shared_ptr<ob::DeviceList>& list) {
  ...
  try {
    ...
    ROS_INFO_STREAM("pre-init device info: name=" << info->name()
                    << " sn=" << info->serialNumber()
                    << " fw=" << info->firmwareVersion()
                    << " uid=" << info->uid());
    auto sensor_list = device->getSensorList();
    ROS_INFO_STREAM("pre-init sensor count=" << sensor_list->count());
    for (size_t i = 0; i < sensor_list->count(); ++i) {
      auto sensor = sensor_list->getSensor(i);
      ROS_INFO_STREAM("pre-init sensor[" << i << "] type=" << sensor->type());
    }
  } catch (const ob::Error& e) {
    ROS_WARN_STREAM("pre-init probe failed: " << e.getMessage());
  } catch (const std::exception& e) {
    ROS_WARN_STREAM("pre-init probe failed: " << e.what());
  } catch (...) {
    ROS_WARN_STREAM("pre-init probe failed");
  }
