...
void OBCameraNode::setupConfig() {
  ROS_INFO_STREAM("setupConfig: begin");
  stream_name_[DEPTH] = "depth";
  ROS_INFO_STREAM("setupConfig: depth name");
  ...
  ROS_INFO_STREAM("setupConfig: color name");
  ...
  ROS_INFO_STREAM("setupConfig: infra0 name");
  ...
  ROS_INFO_STREAM("setupConfig: infra1 name");
  ...
  ROS_INFO_STREAM("setupConfig: infra2 name");
  ...
  ROS_INFO_STREAM("setupConfig: end");
}
...
void OBCameraNode::setupDevices() {
  ...
  try {
    auto set_bool_if_supported = [this](OBPropertyID property_id, bool value,
                                        const std::string& property_name) {
      if (!device_->isPropertySupported(property_id, OB_PERMISSION_READ_WRITE)) {
        ROS_INFO_STREAM("Skip unsupported property " << property_name);
        return;
      }
      device_->setBoolProperty(property_id, value);
    };
    auto set_int_if_supported = [this](OBPropertyID property_id, int value,
                                       const std::string& property_name) {
      if (!device_->isPropertySupported(property_id, OB_PERMISSION_READ_WRITE)) {
        ROS_INFO_STREAM("Skip unsupported property " << property_name);
        return;
    ...
    if (enable_hardware_d2d_ && device_info_->pid() == GEMINI2_PID) {
      set_bool_if_supported(OB_PROP_DISPARITY_TO_DEPTH_BOOL, true, "DISPARITY_TO_DEPTH");
    }
    ...
    if (device_info_->pid() == GEMINI2_PID) {
      ...
      if (default_precision_level != depth_precision_) {
        set_int_if_supported(OB_PROP_DEPTH_PRECISION_LEVEL_INT, depth_precision_,
                             "DEPTH_PRECISION_LEVEL");
      }
    }

    set_bool_if_supported(OB_PROP_DEPTH_SOFT_FILTER_BOOL, enable_soft_filter_, "DEPTH_SOFT_FILTER");
    set_bool_if_supported(OB_PROP_COLOR_AUTO_EXPOSURE_BOOL, enable_color_auto_exposure_,
                          "COLOR_AUTO_EXPOSURE");
    set_bool_if_supported(OB_PROP_IR_AUTO_EXPOSURE_BOOL, enable_ir_auto_exposure_,
                          "IR_AUTO_EXPOSURE");
    set_bool_if_supported(OB_PROP_LDP_BOOL, enable_ldp_, "LDP");
    int default_soft_filter_max_diff = -1;
    if (device_->isPropertySupported(OB_PROP_DEPTH_MAX_DIFF_INT, OB_PERMISSION_READ_WRITE)) {
      default_soft_filter_max_diff = device_->getIntProperty(OB_PROP_DEPTH_MAX_DIFF_INT);
    }
    if (soft_filter_max_diff_ != -1 && default_soft_filter_max_diff != soft_filter_max_diff_) {
      set_int_if_supported(OB_PROP_DEPTH_MAX_DIFF_INT, soft_filter_max_diff_,
                           "DEPTH_MAX_DIFF");
    }
    int default_soft_filter_speckle_size = -1;
    if (device_->isPropertySupported(OB_PROP_DEPTH_MAX_SPECKLE_SIZE_INT,
                                     OB_PERMISSION_READ_WRITE)) {
      default_soft_filter_speckle_size =
          device_->getIntProperty(OB_PROP_DEPTH_MAX_SPECKLE_SIZE_INT);
    }
    if (soft_filter_speckle_size_ != -1 &&
        default_soft_filter_speckle_size != soft_filter_speckle_size_) {
      set_int_if_supported(OB_PROP_DEPTH_MAX_SPECKLE_SIZE_INT, soft_filter_speckle_size_,
                           "DEPTH_MAX_SPECKLE_SIZE");
    }
