...
void OBCameraNode::getParameters() {
  ...
  save_image_count_ = nh_private_.param<int>("save_image_count", 100);
...
void OBCameraNode::publishDepthPointCloud(const std::shared_ptr<ob::FrameSet>& frame_set) {
  ...
  if (!points) {
    ROS_ERROR_STREAM("points is null ");
    return;
  }
...
void OBCameraNode::onNewFrameCallback(const std::shared_ptr<ob::Frame>& frame,
  ...
  if (frame->type() == OB_FRAME_COLOR && frame->format() != OB_FORMAT_RGB888) {
    ...
    if (covert_frame == nullptr) {
      ROS_ERROR_STREAM("Format " << OBFormatToString(frame->format())
                                 << " convert to RGB888 failed");
      return;
    }
...
void OBCameraNode::saveImageToFile(const stream_index_pair& stream_index, const cv::Mat& image,
  ...
  if (save_images_[stream_index] && remaining_images_[stream_index] > 0) {
    auto now = std::chrono::system_clock::now();
    auto now_ms = std::chrono::time_point_cast<std::chrono::milliseconds>(now);
    auto value = now_ms.time_since_epoch().count();
    auto current_path = boost::filesystem::current_path().string();
    auto fps = fps_[stream_index];
    std::string filename = current_path + "/image/" + std::to_string(value) + "_" +
                           stream_name_[stream_index] + "_" + std::to_string(image_msg->width) +
                           "x" + std::to_string(image_msg->height) + "_" + std::to_string(fps) +
                           "hz_.png";
    ...
    remaining_images_[stream_index] -= 1;
  } else {
