...
class PlanLoader:
    ...
    def get_zone_meta(self, zone_id: str, *, map_name: str = "", map_version: str = "") -> Optional[Dict[str, Any]]:
        ...
        try:
            return self.store.get_zone_meta(str(zone_id), map_name=str(map_name or ""), map_version=str(map_version or ""))
        except Exception:
            return None
    ...
    def load_for_zone(
        ...
        *,
        ...
        if not plan_id:
            scope = f" map={map_name}@{map_version}" if (map_name or map_version) else ""
        ...
        return LoadedPlan(
            ...
            map_version=str(meta.get("map_version") or ""),
