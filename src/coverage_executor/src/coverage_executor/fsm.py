...
# ---------- AI spot override (inspection/patrol) ----------
    ...
    def _update_active_run_context(
        ...
        *,
        ...
        if not run_id:
            return
        map_name, map_version, _scope_ok = self._ensure_runtime_map_scope()
        if not map_name:
            map_name = str(getattr(plan, "map_name", "") or "").strip()
        if not map_version:
            map_version = str(getattr(plan, "map_version", "") or "").strip()
        try:
    ...
    def _get_run_snapshot_map_scope(self, run_id: str = "") -> Tuple[str, str]:
        ...
        if run is None:
            return "", ""
        return str(run.map_name or "").strip(), str(run.map_version or "").strip()

    def _ensure_runtime_map_scope(self) -> Tuple[str, str, bool]:
        map_name = ""
        map_version = ""
        ok = False
        try:
            map_name, map_version = get_runtime_map_scope()
            ok = bool(map_name or map_version)
        except Exception:
            ok = False
        ...
        with self._lock:
            ...
            self._runtime_map_version = map_version
        return map_name, map_version, bool(ok)

    def _resolve_plan_lookup_scope(self) -> Tuple[str, str]:
        map_name, map_version = self._get_run_snapshot_map_scope()
        if map_name:
            return map_name, map_version
        map_name, map_version, _ok = self._ensure_runtime_map_scope()
        return map_name, map_version

    def _resolve_expected_map_scope(self, plan: LoadedPlan) -> Tuple[str, str]:
        map_name, map_version = self._get_run_snapshot_map_scope()
        plan_name = str(getattr(plan, "map_name", "") or "").strip()
        plan_version = str(getattr(plan, "map_version", "") or "").strip()
        if not map_name:
            map_name = plan_name
        if not map_version:
            map_version = plan_version
        if not map_name or not map_version:
            runtime_name, runtime_version, _ok = self._ensure_runtime_map_scope()
            if not map_name:
                map_name = runtime_name
            if not map_version:
                map_version = runtime_version
        return str(map_name or "").strip(), str(map_version or "").strip()
    ...
    def _check_zone_consistency_or_abort(self, plan: LoadedPlan) -> bool:
        ...
        """
        try:
            scope_name, scope_version = self._resolve_expected_map_scope(plan)
            zm = self.loader.get_zone_meta(
                str(plan.zone_id),
                map_name=str(scope_name or ""),
                map_version=str(scope_version or ""),
            )
            if not zm:
                return True
    ...
    def _check_map_consistency_or_abort(self, plan: LoadedPlan, ckpt: Optional[dict] = None) -> bool:
        ...
        exp_name, exp_version = self._resolve_expected_map_scope(plan)
        ...
        if (not exp_name) or (not exp_version) or (not exp_id) or (not exp_md5):
            try:
                zm = self.loader.get_zone_meta(
                    str(plan.zone_id),
                    map_name=str(exp_name or ""),
                    map_version=str(exp_version or ""),
                ) or {}
                if not exp_name:
                    exp_name = str(zm.get("map_name") or "").strip()
                if not exp_version:
                    exp_version = str(zm.get("map_version") or "").strip()
                if not exp_id:
        ...
        if (not exp_name and not exp_version) and (not exp_id and not exp_md5):
            # still try to inject for later plans/checkpoints
            self._ensure_runtime_map_scope()
            self._ensure_runtime_map_identity()
            return True

        runtime_name, runtime_version, scope_ok = self._ensure_runtime_map_scope()
        mid, mmd5, ok = self._ensure_runtime_map_identity()

        if strict and ((exp_name and not runtime_name) or (exp_version and not runtime_version) or ((exp_name or exp_version) and not scope_ok)):
            msg = (
                "runtime_map_scope_missing: map_name='%s' map_version='%s' expected_name='%s' expected_version='%s'"
                % (runtime_name, runtime_version, exp_name, exp_version)
            )
            rospy.logerr("[EXEC] %s", msg)
        ...
        if exp_version and runtime_version and exp_version != runtime_version:
            msg = f"map_version_mismatch: expected={exp_version} runtime={runtime_version} plan_id={plan.plan_id} zone={plan.zone_id}"
            if strict:
                rospy.logerr("[EXEC] %s", msg)
                self._emit(f"ERROR:MAP_MISMATCH:{msg}")
                self._set_error(
        ...
        rospy.loginfo(
            "[EXEC] map consistency ok: map=%s@%s map_id=%s map_md5=%s",
            runtime_name,
            runtime_version,
            mid,
            mmd5,
    ...
    def _run_start(self, zone_id: str):
        ...
        map_name, map_version = self._resolve_plan_lookup_scope()
