...
def _iter_candidate_prefixes():
    ...
    for prefix in str(os.environ.get("CMAKE_PREFIX_PATH", "")).split(os.pathsep):
...
def _resolve_binary_path(explicit_path, package_name, binary_name):
    path = os.path.abspath(os.path.expanduser(str(explicit_path or "").strip()))
    if path:
    ...
    checked = []
    for prefix in _iter_candidate_prefixes():
...
class CartographerSupervisorNode:
    def __init__(self):
        ...
        self.cartographer_node_bin = _resolve_binary_path(
            rospy.get_param("~cartographer_node_bin", ""),
            "cartographer_ros",
            "cartographer_node",
        )
        self.occupancy_grid_node_bin = _resolve_binary_path(
            rospy.get_param("~occupancy_grid_node_bin", ""),
            "cartographer_ros",
