...
public:
  ...
  /**
   * @brief Restore the swept cost of the committed incoming edge.
   *
   * Queue entries snapshot this value together with the parent and motion
   * primitive.  It is deliberately not a reusable node-validity cache: for a
   * continuous-coordinate lattice the same discrete node index may be reached
   * through geometrically different edges.
   */
...
private:
  // Maximum cost of the currently evaluated/committed incoming primitive.
  // This is edge state, not a node-index collision cache.
  float _cell_cost;
