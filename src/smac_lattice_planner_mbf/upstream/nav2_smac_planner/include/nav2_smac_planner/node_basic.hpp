...
public:
  ...
  : graph_node_ptr(nullptr),
    parent_ptr(nullptr),
    prim_ptr(nullptr),
    incoming_edge_cost(std::numeric_limits<float>::quiet_NaN()),
    index(new_index),
    motion_index(0u),
    backward(false),
  ...
  void populateSearchNode(NodeT * & node)
  {
    if constexpr (std::is_base_of_v<NodeLattice, NodeT>) {
      ...
      this->parent_ptr = node->parent;
      this->prim_ptr = node->getMotionPrimitive();
      this->incoming_edge_cost = node->getCost();
  ...
  void processSearchNode()
  {
    if constexpr (std::is_base_of_v<NodeLattice, NodeT>) {
      ...
      if (!this->graph_node_ptr->wasVisited()) {
        ...
        this->graph_node_ptr->parent = this->parent_ptr;
        this->graph_node_ptr->setMotionPrimitive(this->prim_ptr);
        this->graph_node_ptr->setCost(this->incoming_edge_cost);
  ...
  NodeT * parent_ptr;  // Used by NodeLattice to keep the queued edge atomic
  MotionPrimitive * prim_ptr;  // Used by NodeLattice
  float incoming_edge_cost;  // Used by NodeLattice
  uint64_t index;
