...
public:
  ...
  struct AnalyticExpansionNodes
  {
    ...
    void setForwardOnlyPreferred(bool preferred)
    {
      forward_only_preferred = preferred;
    }

    std::vector<AnalyticExpansionNode> nodes;
    int direction_changes{0};
    bool forward_only_preferred{false};
  };
  ...
  /**
   * @brief Set an optional domain for analytic-path centre samples.
   * @param center_domain Callback in continuous costmap-cell coordinates.
   */
  ...
  /**
   * @brief Prefer a forward-only analytic connection while preserving a
   *        penalized lattice-search fallback when reverse is required.
   */
  AnalyticExpansionNodes getPreferredAnalyticPath(
