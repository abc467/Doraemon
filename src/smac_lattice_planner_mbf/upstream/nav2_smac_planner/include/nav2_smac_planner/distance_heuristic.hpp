...
// Mirroring a heading across the X axis maps bin 0 back to bin 0.  The raw
// expression `num_bins - bin` produces the out-of-range value `num_bins` for
// that common case, so keep the wrap explicit and independently testable.
inline unsigned int mirrorAngleBin(
  const unsigned int bin, const unsigned int num_bins)
{
  return num_bins == 0u ? 0u : (num_bins - (bin % num_bins)) % num_bins;
}
