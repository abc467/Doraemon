...
template<typename NodeT>
void AnalyticExpansion<NodeT>::setCenterDomain(CenterDomainCallback center_domain)
{
...
template<typename NodeT>
typename AnalyticExpansion<NodeT>::AnalyticExpansionNodes
AnalyticExpansion<NodeT>::getPreferredAnalyticPath(
  const NodePtr & node,
  const NodePtr & goal,
  const NodeGetter & getter)
{
  if constexpr (std::is_base_of_v<Node2D, NodeT>) {
    ...
    if (_search_info.prefer_forward_analytic_expansion &&
      _search_info.allow_reverse_expansion &&
      (_motion_model == MotionModel::STATE_LATTICE ||
      _motion_model == MotionModel::REEDS_SHEPP))
    {
      const auto forward_state_space =
        std::make_shared<ompl::base::DubinsStateSpace>(
        _ctx->motion_table.min_turning_radius);
      auto forward_nodes = getAnalyticPath(
        node, goal, getter, forward_state_space);
      if (!forward_nodes.nodes.empty()) {
        forward_nodes.setForwardOnlyPreferred(true);
        return forward_nodes;
      }
      // Do not immediately accept the shortest Reeds-Shepp completion here:
      // analytic expansion scoring does not include reverse_penalty. Returning
      // no shortcut lets A* keep exploring both forward and reverse lattice
      // primitives, where the configured penalties are applied consistently.
      return AnalyticExpansionNodes();
    }
    return getAnalyticPath(
      node, goal, getter, _ctx->motion_table.state_space);
  }
}
  ...
  const ompl::base::StateSpacePtr & state_space)
{
  ...
  if constexpr (std::is_base_of_v<Node2D, NodeT>) {
    ...
    float previous_x = static_cast<float>(from[0]);
    float previous_y = static_cast<float>(from[1]);
    double previous_yaw = static_cast<double>(from[2]);
    ...
    for (float i = 1; i <= num_intervals; i++) {
      ...
      // Analytic shortcuts must obey the same centre-domain constraint as
      // discrete lattice neighbors. Check continuous samples before collision
      // evaluation or graph insertion so a shortcut cannot leave the corridor.
      if (_center_domain &&
        !_center_domain(static_cast<float>(reals[0]), static_cast<float>(reals[1])))
      {
        failure = true;
        break;
      }
      ...
      // Validate the full rigid-body sweep between analytic samples. This is
      // especially important for a long rectangular differential-drive base:
      // checking only endpoint footprints can miss a front-corner collision.
      float analytic_segment_cost = FREE_COST;
      if (_collision_checker->inCollisionContinuous(
          previous_x, previous_y, previous_yaw,
          static_cast<float>(reals[0]), static_cast<float>(reals[1]), theta,
          _traverse_unknown, &analytic_segment_cost))
      {
        failure = true;
        break;
      }
      previous_x = static_cast<float>(reals[0]);
      previous_y = static_cast<float>(reals[1]);
      previous_yaw = theta;
  ...
  if constexpr (std::is_base_of_v<Node2D, NodeT>) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        test_node->parent->parent->parent->parent->parent)
      {
        ...
        if (analytic_nodes.forward_only_preferred) {
          const auto forward_state_space =
            std::make_shared<ompl::base::DubinsStateSpace>(
            _ctx->motion_table.min_turning_radius);
          refined_analytic_nodes = getAnalyticPath(
            test_node, goal_node, getter, forward_state_space);
          refined_analytic_nodes.setForwardOnlyPreferred(
            !refined_analytic_nodes.nodes.empty());
        } else {
          refined_analytic_nodes = getAnalyticPath(
    ...
    while (min_turn_rad < max_min_turn_rad) {
      ...
      if (_ctx->motion_table.motion_model == MotionModel::DUBIN ||
        analytic_nodes.forward_only_preferred)
      {
