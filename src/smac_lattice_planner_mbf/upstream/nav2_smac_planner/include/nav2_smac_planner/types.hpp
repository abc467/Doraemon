...
struct SearchInfo
{
  ...
  // For differential-drive lattices with reverse primitives enabled, first
  // attempt a forward-only Dubins analytic connection. If it is unavailable,
  // the ordinary lattice search continues; its reverse primitives remain the
  // penalized fallback instead of accepting an unscored Reeds-Shepp shortcut.
  bool prefer_forward_analytic_expansion{false};
...
struct SmootherParams
{
  ...
  // ROS parameter loading is provided by the ROS 1 MBF adapter. The optional
  // Nav2 smoother is not built in this package, but this upstream type remains
  // available to the search headers.
