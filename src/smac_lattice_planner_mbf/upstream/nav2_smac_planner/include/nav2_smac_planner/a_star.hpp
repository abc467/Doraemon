...
/** @brief Why an A* invocation stopped. */
...
/** @brief Search outcome, including tolerance paths returned at a terminal limit. */
struct SearchResult
{
  bool path_found{false};
  SearchTermination termination{SearchTermination::OPEN_EXHAUSTED};

  bool hasPath() const noexcept {return path_found;}
};
...
public:
  ...
  /**
   * @brief Create a path and report the precise reason the search terminated.
   *
   * Unlike the legacy bool wrapper, cancellation is returned as CANCELED
   * instead of throwing. A tolerance path can be returned together with the
   * TIMEOUT, ITERATION_LIMIT, or OPEN_EXHAUSTED reason that ended refinement.
   */
  SearchResult createPathDetailed(
    CoordinateVector & path, int & num_iterations, const float & tolerance,
    std::function<bool()> cancel_checker,
    std::vector<std::tuple<float, float, float>> * expansions_log = nullptr);

  /**
   * @brief Restrict graph-node centres to an optional 2-D planning domain.
   *
   * Coordinates are expressed in continuous costmap-cell coordinates. An
   * empty callback (the default) is the unrestricted FULL domain.
   */
  void setCenterDomain(CenterDomainCallback center_domain);

  /** @brief Restore the unrestricted FULL planning domain. */
  void clearCenterDomain();

  /**
   * @brief Add an optional 2-D lower-priority guide to A* queue ordering.
   *
   * The callback receives continuous costmap-cell coordinates and must return
   * a finite, non-negative cost in lattice-cell units. Queue ordering uses
   * max(anchor, callback), while goal tolerance and closest-node bookkeeping
   * continue to use only the official obstacle/motion-model anchor.
   */
  void setAdditionalHeuristic(AdditionalHeuristicCallback additional_heuristic);
   ...
   *
   * The default empty callback preserves the upstream search exactly. This is
   * deliberately applied before the goal node is queued, so rejecting one
   * incoming primitive does not visit the state and suppress a later valid
   * incoming primitive to the same SE(2) index.
   */
  void setGoalTransitionValidator(GoalTransitionValidator validator);

  /** @brief Restore official goal-transition behavior. */
  void clearGoalTransitionValidator();
  ...
  void setStart(
    ...
    const float & my,
    const unsigned int & dim_3,
    const float & initial_cost = 0.0f);

  /**
   * @brief Add another collision-checked SE(2) search seed at the same physical start.
   * @param mx Continuous map X coordinate
   * @param my Continuous map Y coordinate
   * @param dim_3 Heading bin
   * @param initial_cost Cost of correcting the exact heading into this bin
   *
   * State lattices quantize heading. Multiple safe adjacent seeds prevent the
   * nearest-bin rounding direction from being the only possible way to enter
   * the lattice near an asymmetric obstacle.
   */
  void addStart(
    const float & mx,
    const float & my,
...
protected:
  ...
  NodePtr _start;
  std::vector<std::pair<NodePtr, float>> _starts;
