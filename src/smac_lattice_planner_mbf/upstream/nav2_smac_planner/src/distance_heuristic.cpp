  ...
  MotionTableT & motion_table)
{
  ...
  if (abs(node_coords_relative.x) < floored_size && mirrored_relative_y < floored_size) {
    ...
    unsigned int theta_pos;
    if (node_coords_relative.y < 0.0) {
      theta_pos = mirrorAngleBin(
        static_cast<unsigned int>(node_coords_relative.theta),
        motion_table.num_angle_quantization);
    } else {
      theta_pos = static_cast<unsigned int>(node_coords_relative.theta) %
        motion_table.num_angle_quantization;
    }
