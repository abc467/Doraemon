...
void ObstacleHeuristic::resetObstacleHeuristic(
  nav2_costmap_2d::Costmap2D * costmap_i,
    ...
    for (unsigned int i = 0; i != neighborhood.size(); i++) {
      ...
      if (new_idx < size_x * size_y) {
        if (downsample_obstacle_heuristic) {
          ...
          const unsigned int map_y = new_idx / costmap->getSizeInCellsX();
          const unsigned int map_x = new_idx - map_y * costmap->getSizeInCellsX();
          cost = static_cast<float>(costmap->getCost(map_x, map_y));
        }
