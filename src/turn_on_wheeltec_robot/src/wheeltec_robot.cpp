...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  private_nh.param<std::string>("car_mode",         car_mode,    "mini_mec"); //机器人型号
  private_nh.param<std::string>("arm_cmd", JointStates_topic, "arm_cmd");

  robot_type = car_mode.substr(0,4); 
  if(robot_type!="mini")
    robot_type="Plus";
  else
    robot_type="Mini";
...
turn_on_robot::~turn_on_robot()
{
  ...
  const short arm_a = static_cast<short>(1.5707*1000);
  Send_Data.tx[3]=arm_a>>8;
  Send_Data.tx[4]=arm_a&0xff;

  const short arm_b = static_cast<short>(0.3917*1000);
  Send_Data.tx[5]=arm_b>>8;
  Send_Data.tx[6]=arm_b&0xff;
