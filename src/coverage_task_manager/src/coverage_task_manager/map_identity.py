# -*- coding: utf-8 -*-
"""Map identity utilities (task layer).

Task layer is the natural place to guarantee system-wide map identity
(map_id/map_md5) because it usually starts early and orchestrates other modules.

This module duplicates logic to avoid cross-package dependencies.
"""
...
def get_runtime_map_scope() -> Tuple[str, str]:
    map_name = _normalize_map_name(_get_param_first(["~map_name", "/map_name"]))
...
def ensure_map_identity(
    *,
    ...
    ok = True
    updated = bool(refresh)
    if refresh or (not map_md5):
