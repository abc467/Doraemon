...
bool ValidateLaserScanTiming(const std::string &topic, const ros::Time &stamp,
							 const double scan_time, const double time_increment,
							 const int point_count)
{
	static std::map<std::string, ros::Time> last_scan_stamps;
	if (point_count <= 0 || stamp.isZero() || !std::isfinite(scan_time) ||
		!std::isfinite(time_increment) || scan_time <= 0.0 ||
		time_increment <= 0.0)
	{
		ROS_WARN_THROTTLE(2.0,
						  "Drop LaserScan on %s because timing is invalid: "
						  "stamp=%.9f scan_time=%.9f time_increment=%.12f points=%d",
						  topic.c_str(), stamp.toSec(), scan_time,
						  time_increment, point_count);
		return false;
	}

	ros::Time &last_stamp = last_scan_stamps[topic];
	if (!last_stamp.isZero() && stamp <= last_stamp)
	{
		ROS_WARN_THROTTLE(2.0,
						  "Drop LaserScan on %s because timestamp is not "
						  "monotonic: stamp=%.9f last=%.9f delta=%.9f",
						  topic.c_str(), stamp.toSec(), last_stamp.toSec(),
						  (stamp - last_stamp).toSec());
		return false;
	}
	last_stamp = stamp;
	return true;
...
void PublishLaserScanFan(ros::Publisher &laser_pub, RawData *fan, std::string &frame_id, double min_dist, double max_dist, uint8_t inverted, uint8_t reversed)
{
	...
	if (!ValidateLaserScanTiming(laser_pub.getTopic(), msg.header.stamp,
								 msg.scan_time, msg.time_increment, fan->N))
	{
		return;
	}
...
void PublishLaserScan(ros::Publisher &laser_pub, HPublish pub, ArgData argdata, int8_t counterclockwise)
{
	...
	{
		ROS_WARN_THROTTLE(2.0,
						  "Drop LaserScan on %s because point count is zero.",
						  laser_pub.getTopic().c_str());
		return;
	}
	...
	if (!ValidateLaserScanTiming(laser_pub.getTopic(), msg.header.stamp,
								 msg.scan_time, msg.time_increment, N))
	{
		return;
	}
