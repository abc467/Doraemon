# Doraemon x86 Ubuntu 20.04 v10 发布说明

固定发布组合：

- 后端：`deployment-2026-07-27-x86-ubuntu20-v10`
- 前端：`deployment-2026-07-22-frontend-v3`
- 前端版本：`0.1.0-rc.11`

## 本版变更

- 商业部署账号策略改为公司批准的固定角色口令，不再要求强口令或逐车随机口令。
  作为网络补偿门禁，Site Gateway 必须精确绑定登记的现场管理地址；禁止监听
  `0.0.0.0`、机器人内部设备网地址或公网地址。9090/11311 仍保持 loopback-only。
...
- 无法完整扫描 `planning.db` / `operations.db`、固定审计表或列缺失时，地图物理
  ...
  候选扫描后新出现的引用被静默孤立。
- v10 完整继承 v9 的 rosbridge 回环监听、Orbbec 枚举、M-core 动作链、地图
  revision 切换和商业启动门禁；未放宽既有安全边界。
- 商业部署手册、固定版本 manifest 和不可变发布测试已统一到上述后端/前端组合及
  新账号/管理网策略。
...
- 本地标签未推送远端时只能作为候选基线，不得宣称已完成公司制品库发布，也不能删除
  逐车未发布标签例外。正式构建必须先把注解标签发布到规范远端，再从该远端重新浅克隆。
